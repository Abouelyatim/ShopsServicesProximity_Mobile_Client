    ...
    class AllProductHolder(
        ...
        fun bind(position: Int) = with(itemView) {
            if(selectedPosition==position){
                itemView.custom_category_container.background= ResourcesCompat.getDrawable(resources,R.drawable.raduis_selector_selected_bleu,null)
                itemView.view_custom_category_name.setTextColor(Color.parseColor("#ffffff"))
            }
            else{
                itemView.custom_category_container.background=ResourcesCompat.getDrawable(resources,R.drawable.raduis_selector_not_selected_white,null)
                itemView.view_custom_category_name.setTextColor(Color.parseColor("#000000"))
            }

            itemView.setOnClickListener {
                selectedPosition=position
                interactionAll?.onItemAllSelected()
            }
